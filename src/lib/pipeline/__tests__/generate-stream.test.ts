...
jest.mock('../../sources/gather', () => ({
  gatherSources: jest.fn(async () => ({
    sources: [{ source: 'web_search', items: [{ title: 'a', content: 'b' }], queryCount: 1, elapsedMs: 1 }],
    totalItems: 1,
    elapsedMs: 1,
  })),
}));
...
describe('generateRoutes stage callback', () => {
  test('emits stage updates through callback', async () => {
    const stages: string[] = [];

    await generateRoutes(
      { origin: 'Ho Chi Minh City', destination: 'Vung Tau' },
      (stage) => {
